...
export function ImagesManagement({
  ...
  const [currentImage, setCurrentImage] = useState<Partial<SubsectorImage>>({
    is_published: true
  });
  ...
  const handleOpenModal = (image?: SubsectorImage) => {
    if (image) {
    ...
    try {
      ...
      if (!isEditing && selectedFile) {
        ...
        const endpoint = isEditing 
          ? `/api/admin/subsectors/${subsectorId}/images/${currentImage.id}`
          : `/api/admin/subsectors/${subsectorId}/images`;
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      // Iniciar upload
      xhr.open('POST', `/api/admin/subsectors/${subsectorId}/images/upload`);
      xhr.send(formData);
  ...
  const handleDeleteImage = async (imageToDelete: SubsectorImage) => {
    try {
  ...
  const filteredImages = showDrafts 
    ? images.filter(img => !img.is_published)
    : images.filter(img => img.is_published);
      ...
      {previewImageId && (
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="max-w-4xl max-h-full">
            {(() => {
              const image = filteredImages.find(img => img.id === previewImageId);
