...
const CreateGroupModal: React.FC<CreateGroupModalProps> = ({
  ...
  // Load data function - defined before useEffect to avoid dependency issue
  ...
  const filteredUsers = useMemo(() => {
    ...
    if (selectedSectors.length > 0) {
      filtered = filtered.filter(user => 
        user.sector_id && selectedSectors.includes(user.sector_id)
      );
    }

    if (selectedPositions.length > 0) {
      filtered = filtered.filter(user => 
        selectedPositions.some(pos => user.position?.includes(pos))
      );
    }
  ...
  const handleSubmit = async () => {
    ...
    if (filteredUsers.length === 0) {
      setError('Selecione pelo menos um filtro para incluir usuários no grupo');
      return;
    }
                  ...
                  {filteredUsers.length === 0 ? (
                    <div className="text-center py-4">
                      <p className="text-sm text-gray-500">
                        Selecione setores e/ou cargos para filtrar usuários
                      </p>
                    </div>
                  ) : (
                      ...
                      {filteredUsers.length > 50 && (
                        <div className="text-center py-2 border-t">
                          <p className="text-xs text-gray-500">
                            ... e mais {filteredUsers.length - 50} usuários
                          </p>
                        </div>
                      )}
                    </div>
