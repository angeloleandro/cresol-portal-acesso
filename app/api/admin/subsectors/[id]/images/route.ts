...
// GET /api/admin/subsectors/[id]/images - Listar imagens do subsetor
export async function GET(
  ...
  try {
    ...
    if (!profile || !['admin', 'sector_admin', 'subsector_admin'].includes(profile.role)) {
      return NextResponse.json(
        { error: 'Sem permissão para acessar imagens do subsetor' },
        { status: 403 }
      );
    }
    ...
    console.error('Erro ao buscar imagens do subsetor:', error);
    return NextResponse.json(
...
// POST /api/admin/subsectors/[id]/images - Criar nova imagem
