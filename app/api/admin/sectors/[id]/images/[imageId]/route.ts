...
// PUT /api/admin/sectors/[id]/images/[imageId] - Atualizar imagem
export async function PUT(
  ...
  try {
    ...
    // Verificar se a imagem existe e pertence ao setor
...
// DELETE /api/admin/sectors/[id]/images/[imageId] - Excluir imagem
export async function DELETE(
  ...
  try {
    ...
    // Verificar se a imagem existe e pertence ao setor, e obter dados para limpeza
    const { data: existingImage, error: checkError } = await supabase
      .from('sector_images')
    ...
    if (existingImage.thumbnail_url && 
      ...
      if (thumbnailPath) {
        await supabase.storage
          .from('images')
          .remove([`thumbnails/sectors/${thumbnailPath}`]);
      }
    }
