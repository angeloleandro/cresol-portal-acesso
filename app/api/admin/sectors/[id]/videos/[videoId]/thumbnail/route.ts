...
// POST /api/admin/sectors/[id]/videos/[videoId]/thumbnail - Upload de thumbnail
export async function POST(
  ...
  try {
    ...
    if (existingVideo.thumbnail_url && existingVideo.thumbnail_url.includes('supabase')) {
      ...
      if (oldPath) {
        ...
        await serviceClient.storage
          .from('images')
          .remove([`thumbnails/sectors/${sectorId}/${oldPath}`]);
      }
    }
    ...
    const filePath = `thumbnails/sectors/${sectorId}/${fileName}`;
