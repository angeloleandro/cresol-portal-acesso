...
export function DocumentsManagement({
  ...
  const [currentDocument, setCurrentDocument] = useState<Partial<SectorDocument>>({
    title: '',
    description: '',
    file_url: '',
    file_type: '',
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!ALLOWED_FILE_TYPES.includes(file.type as any)) {
        showError('Arquivo inválido', 'Por favor, selecione um arquivo permitido (PDF, Word, Excel, PowerPoint, TXT, ZIP, RAR)');
        return;
      }

      // Validar tamanho
      if (file.size > MAX_FILE_SIZE) {
        showError('Arquivo muito grande', `O arquivo deve ter no máximo ${MAX_FILE_SIZE_LABEL}`);
        return;
      }
  ...
  const uploadFile = async (): Promise<string | null> => {
    if (!documentFile) return currentDocument.file_url || null;
  ...
  const handleOpenModal = (documentItem?: SectorDocument) => {
    if (documentItem) {
      setCurrentDocument(documentItem);
      setIsEditing(true);
      // Set input type based on existing document
      setInputType('url'); // Default to URL for existing documents
    } else {
      setCurrentDocument({
        title: '',
        description: '',
      ...
      setIsEditing(false);
      setInputType('url'); // Default to URL for new documents
    }
    setDocumentFile(null);
    setIsModalOpen(true);
  };
  ...
  const handleSaveDocument = async (e: React.FormEvent) => {
    ...
    // Validação específica baseada no tipo de input
    if (inputType === 'url') {
  ...
  const handleDeleteClick = (documentItem: SectorDocument) => {
    deleteModal.openDeleteModal(documentItem, documentItem.title);
  };

  const handleDeleteDocument = async (documentItem: SectorDocument) => {
    try {
                  ...
                  <div>
                    ...
                    {documentFile ? (
                      ...
                      <div>
                        ...
                        />
                        <p className="text-xs text-gray-500 mt-1">
                          Tipos permitidos: PDF, Word, Excel, PowerPoint, TXT, ZIP, RAR (máximo {MAX_FILE_SIZE_LABEL})
                        </p>
                      </div>
                    )}
                  </div>
