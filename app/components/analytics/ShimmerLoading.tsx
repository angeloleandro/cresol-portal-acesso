...
export default function ShimmerLoading({ 
  ...
  const renderShimmerChart = () => (
    <div className="bg-white rounded-md border border-gray-200 p-6 animate-pulse">
      <div className="h-6 bg-gray-200 rounded-sm w-48 mb-6"></div>
      <div className="space-y-4">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={index} className="flex items-center space-x-4">
            <div className="w-16 h-4 bg-gray-200 rounded-sm"></div>
            <div className="flex-1 h-4 bg-gray-200 rounded-sm"></div>
      ...
      <div className="space-y-3">
        {Array.from({ length: 5 }).map((_, index) => (
          <div key={index} className="flex items-center justify-between">
            ...
            <div className="h-4 bg-gray-200 rounded-sm w-8"></div>
          </div>
        ))}
      </div>
    </div>
