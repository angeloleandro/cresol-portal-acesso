...
// Helper function to check if video is ready to play
function isVideoReady(video: DashboardVideo): boolean {
...
export default function VideoGalleryEnhanced({ limit = 4 }: VideoGalleryProps) {
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      ...
      const readyVideos = (data || []).filter(isVideoReady);
  ...
  if (showVideos.length === 1) {
    ...
    const videosToShow = Math.min(showVideos.length, 4);
    content = (
      <>
...
// Componente de Card de Vídeo com indicadores visuais para tipos de upload
